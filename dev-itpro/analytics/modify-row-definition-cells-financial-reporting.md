...
-   Etykieta musi zaczynać od litery (a-z lub A-Z) i może być dowolną kombinacją maksymalnie 16 cyfr i liter. **Uwaga:** etykieta może zawierać znaki podkreślenia (\_), ale nie inne znaki specjalne są niedozwolone.
...
Komórka **Kod formatu** pozwala wybrać wstępnie sformatowane elementy zawartości tego wiersza. Jeśli komórka **Kod formatu** jest pusta, wiersz jest interpretowany jako wiersz szczegółów danych finansowych. **Uwaga:** Jeśli raport zawiera wiersze z formatowaniem niekwotowym, które są powiązane z wierszami kwoty, które zostały pominięte (np. z powodu zerowych sald), można użyć kolumny **Powiązane formuły/wiersze/jednostki**, aby zapobiec drukowaniu wierszy tytułu i formatu.
...
3.  Wybierz kod formatu z listy. Poniższa tabela opisuje kody formatów i związane z nimi akcje.
    ...
    | TOT                           | Suma                              | Określa wiersz używający operatorów matematycznych w kolumnie **Powiązane formuły/wiersze/jednostki**. Sumy zawierają proste operatorów, takich jak **+**lub**-**.                                                      |
    | CAL                           | Obliczenie                        | Określa wiersz używający operatorów matematycznych w kolumnie **Powiązane formuły/wiersze/jednostki**. Obliczenia zawierają złożone operatory, takie jak **+**, **-**, **\***, **/**, i **ELSE-IF/THEN** instrukcji. |
...
W **kod formatu** kolumny w definicji wiersza, **DES**, **LFT**, **RGT**, **CEN**, **---**, i **===**kody formatów zastosować formatowanie do wierszy — kwota. Aby to formatowanie nie było drukowane, gdy powiązane wiersze kwoty są pominięte (np. z powodu wartości zerowych w wierszach kwoty lub braku aktywności w okresie), trzeba powiązać wiersze formuły z odpowiednimi wierszami kwoty. Ta funkcja jest przydatna, jeśli chcesz zapobiec drukowaniu nagłówków lub formatowania sum częściowych, gdy dany okres nie zawiera informacji do wydrukowania. **Uwaga:** można również zapobiegać drukowaniu szczegółowych wierszy kwoty, usuwając zaznaczenie opcji wyświetlania wierszy bez kwot. Ta opcja znajduje się na karcie **Ustawienia** w definicji raportu. Domyślnie konta szczegółów transakcji, które mają zerowe saldo lub nie mają aktywności w okresie, są blokowane w raportach. Aby wyświetlić te konta szczegółów transakcji, zaznacz pole wyboru **Wyświetlaj wiersze bez kwot** na karcie **Ustawienia** w definicji raportu.

### <a name="relate-a-format-row-to-an-amount-row"></a>Wiązanie wiersza formatu z wierszem kwoty

1.  W Projektancie raportów kliknij **Definicje wiersza**, a następnie wybierz wiersz do zmodyfikowania.
2.  W wierszu formatowania w komórce **Powiązane formuły/wiersze/jednostki** wpisz kod wiersza kwoty, który ma być pomijany. **Uwaga:** aby pomijać wiersze kwoty, saldo wiersza musi mieć wartość 0 (zero). Wiersz kwoty z saldem nie jest pomijany.
3.  W menu **Plik** kliknij **Zapisz**.

### <a name="example-of-preventing-printing-of-rows"></a>Przykład zapobiegania drukowaniu wierszy

W poniższym przykładzie Phyllis chce zapobiec drukowaniu nagłówków i podkreśleń w wierszu **Gotówka — razem**, ponieważ na żadnym z kont kasowych nie było aktywności. W związku z tym w wierszu 220 (co, jako **---**wskazuje kod formatu jest wierszem formatowania), w **powiązane formuły/wiersze/jednostki** komórki, przechodzi ona **250**, który jest kod wiersza, kwota wiersza, który chce, aby pominąć. [![RelatedRowsRowDefinition](./media/relatedrowsrowdefinition-1024x144.png)](./media/relatedrowsrowdefinition.png)
...
6.  W menu **Plik** kliknij **Zapisz jako**, aby zapisać definicję kolumny pod nową nazwą. Dołącz **CBR** do bieżącej nazwy pliku (na przykład **CUR\_od początku roku\_CBR**). Ta definicja kolumny jest definicją kolumny wiersza podstawowego.
...
W poniższym przykładzie definicji wiersza wiersz 100 pokazuje, że wierszem podstawowym dla obliczeń jest wiersz 280. [![Przykład obliczania podstawy wierszy.](./media/cbrrowdefinition.png)](./media/cbrrowdefinition.png) W poniższym przykładzie definicji kolumny w obliczeniach używany jest kod formatu **CBR**. Obliczenia w kolumnie C dzieli wartość w kolumnie B raportu według wartości określonej w wierszu 280 kolumny B. Zmiana formatu w kolumnie B drukuje wynik obliczenia jako wartość procentową. Podobnie każda kwota w kolumnie E jest kwotą w kolumnie D przedstawioną w postaci wartości procentowej sprzedaży netto. [![Przykład definicji kolumny.](./media/cbrcolumndefinition2.png)](./media/cbrcolumndefinition2.png) W poniższym przykładzie pokazano raport, który może zostać wygenerowany na podstawie poprzednich obliczeń. [![Przykład raportu na podstawie poprzedniego przykładu obliczeń.](./media/cbrreport-1024x272.png)](./media/cbrreport.png)
...
4.  W komórce **Ograniczenia kolumny** należy wprowadzić literę kolumny raportu, która ma być użyta do sortowania. **Uwaga:** w obliczeniach sortowania należy uwzględniać tylko wiersze kwoty.

### <a name="examples-of-ascending-and-descending-column-values"></a>Przykłady wartości kolumn w porządku rosnącym i malejącym

W poniższym przykładzie wartości w kolumnie D raportu zostaną posortowane rosnąco dla wierszy 160 za pośrednictwem 490. Dodatkowo wartości bezwzględne w kolumnie G raportu zostaną posortowane w kolejności malejącej dla wierszy 610 za pośrednictwem 940.
...
**Posortowane według odchylenia bezwzględnego od początku roku w kolejności malejącej**
...
-   **Zastąp test z wartością zero** — tekst, który trzeba wstawić w raporcie, gdy kwota wynosi 0 (zero). Ten tekst jest widoczny w ostatnim wierszu w **Próbka**. **Uwaga:** Jeśli drukowanie nie jest pomijane dla wartości zerowych lub braku aktywności w okresie, ten tekst jest pomijany.
...
| 0;(0);-                        | 1235       | 1235         | -       |
| \#,\#\#\#;(\#,\#\#\#);“”       | 1235      | (1235)      | (Puste) |
| \#,\#\#0.00;(\#,\#\#0.00);zero | 1234,56   | (1234,56)   | zero    |
| 0,00%;(0,00%)                  | 123456,00% | (123456,00%) | 0,00%   |

## <a name="specify-a-normal-balance-cell"></a>Określanie komórki zwykłego salda
Komórka **Zwykłe saldo** w definicji wiersza określa znak kwoty w wierszu. Aby zmienić znak wiersza lub jeśli zwykłe saldo konta jest transakcją kredytową, wpisz **C** w komórce **Zwykłe saldo** tego wiersza. Projektant raportu powoduje, że znak na wszystkich kontach bilansowych kredytu w tym wierszu. Gdy projektant raportów konwertuje te konta, usuwa cechę debet/kredyt ze wszystkich kwot, co upraszcza sumowanie. Na przykład aby obliczyć zysk netto, odejmuje się wydatki od przychodów. Zwykle sumowane i obliczone wiersze nie są uwzględnione w kodzie **C**. Jednak formant drukowania **XCR** w definicji kolumny odwraca znak każdego wiersza zawierającego **C** w kolumnie **Zwykłe saldo**. To formatowanie jest szczególnie ważne, jeśli chce się pokazać wszystkie niekorzystna odchylenia jako kwoty ujemne. Jeśli zsumowana lub obliczona liczba ma zły znak, wpisz **C** w komórce **Zwykłe saldo** dla wiersza, aby go odwrócić.
...
5.  W obszarze **Atrybuty** wykonaj następujące kroki, aby dodać wpis dla każdego atrybutu, który powinien być uwzględniony w kodzie wiersza:
    1.  Kliknij dwukrotnie komórkę **Atrybut**, a następnie wybierz nazwę atrybutu. **Uwaga:** zastąpić znak numeru (\#) z wartością liczbową.
...
Po wybraniu określonego konta projektant raportów zwykle łączy konta i lata obrachunkowe, okresy obrachunkowe i inne informacje określone w definicji kolumny. Można używać różnych informacji, takich jak różne okresy obrachunkowe, dla określonych wierszy. W poniższej tabeli przedstawiono dostępne modyfikatory kont. Zamień znak numeru (\#) z wartością, która jest równa lub mniejsza niż liczba okresów w roku obrachunkowym.

| Modyfikator konta | Co jest drukowane                                                                           |
|------------------|-------------------------------------------------------------------------------------------|
| /BB              | Saldo początkowe dla konta.                                                     |
| /\#              | Saldo dla określonego okresu.                                                     |
| /-\#             | Saldo w okresie, który jest \#okresów przed bieżącym okresem.                  |
| /+\#             | Saldo w okresie, który jest \#okresów po bieżącym okresie.                   |
| /C               | Saldo dla bieżącego okresu.                                                       |
| /C-\#            | Saldo w okresie, który jest \#okresów przed bieżącym okresem.                  |
| /C+\#            | Saldo w okresie, który jest \#okresów po bieżącym okresie.                   |
| /Y               | Saldo od początku roku do bieżącego okresu.                                      |
| /Y-\#            | Saldo od początku roku, przez okres, który jest \#okresów przed bieżącym okresem. |
| /Y+\#            | Saldo od początku roku, przez okres, który jest \#okresów po bieżącym okresie.  |

### <a name="book-code-modifiers"></a>Modyfikatory kody księgi

Możesz ograniczyć wiersz do istniejącego kodu księgi. Definicja kolumny musi zawierać co najmniej jedną kolumnę **FD**, która ma kod księgi. **Uwaga:** ograniczenie kodu księgi dla wiersza zastępuje ograniczenia kodu księgi w definicji kolumny dla tego wiersza.
...
5.  W **z** polu umożliwia wprowadzenie wymiaru początkowego lub kliknij ![Przeglądaj](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Przeglądaj") do wyszukiwania dostępnych wymiarów. Aby wprowadzić zakres wymiarów, należy wprowadzić wymiar końcowy w polu **Do**.
...
Po wprowadzeniu wartości segmentu naturalnego w **wymiary** okno dialogowe, można umieścić symbol wieloznaczny znaku (? Lub \*) w dowolnym miejscu segmentu. Projektant raportów wyodrębnia wszystkie wartości dla określonych pozycji bez uwzględniania znaków wieloznacznych. Na przykład definicja wiersza zawiera tylko wartości naturalnych segmentów, a naturalne segmenty mają cztery znaki. Wprowadzając **6???** W wierszu poinstruuj Projektant raportów, aby uwzględnić wszystkie konta, które mają wartość segment naturalny, który zaczyna się od 6. Jeśli wprowadzisz **6\***, te same wyniki są zwracane, ale wyniki również zawierać wartości zmiennej szerokości, takie jak **60** i **600000**. Projektant raportów zastępuje poszczególne symbole wieloznaczne (?) pełnym zakresem możliwych wartości, co obejmuje litery i znaki specjalne. Na przykład w zakresie od **12?0** do **12?4**, symbol wieloznaczny w kombinacji **12?0** jest zastępowany najniższą wartością w zestawie znaków, a symbol wieloznaczny w kombinacji **12? 4** jest zastępowany najwyższą wartością w zestawie znaków. **Uwaga:** należy unikać używania symboli wieloznacznych dla kont na początkach i końcach zakresów. Używanie symboli wieloznacznych w konta początkowych lub końcowych może dawać nieoczekiwane wyniki.

### <a name="single-segment-or-single-dimension-ranges"></a>Zakresy z jednym segmentem lub jednym wymiarem

Można określić zakres wartości segmentów lub wartości wymiarów. Zaletą określania zakresu jest to, że nie trzeba aktualizować definicji wiersza za każdym razem, gdy do danych finansowych dodawana jest nowa wartość segmentu lub wartość wymiaru. Na przykład, zakres **+ konto =\[6100:6900\]** wciąga wartości z kont 6100 do 6900 do kwoty wiersza. Gdy zakres zawiera symbol wieloznaczny (?), projektant raportów nie ocenia zakresu na zasadzie znak po znaku. Zamiast tego określane są niższe i wyższe krańce zakresu, a następnie uwzględniane są wartości krańcowe i wszystkie wartości między nimi. **Uwaga:** projektant raportu nie można wybrać konta, wymiary lub pól z systemu Microsoft Dynamics ERP, który zawierać żadnego z następujących znaków zastrzeżonych: &, \*, \[, \], {, lub}. Można dodać znak „&” tylko w przypadku automatycznego tworzenia definicji wierszy przy użyciu okna dialogowego **Wstawianie wierszy z wymiarów**.

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Zakresy z wieloma segmentami lub wieloma wymiarami

Po wprowadzeniu zakresu przy użyciu kombinacji wielu wartości wymiaru porównanie zakresu odbywa się na... podstawy \financial-dimensions\dimension-by-Dimension. Porównanie zakresu nie może być wykonywane ani na zasadzie znak po znaku ani na podstawie segmentu częściowego. Na przykład, zakres **+ konto =\[5000:6000\], Departament =\[1000:2000\], MPK =\[00\]** dotyczy tylko kont, które odpowiadają każdego segmentu. W tym scenariuszu pierwszy wymiar musi być w zakresie od 5000 do 6000, drugi wymiar musi być w zakresie od 1000 do 2000, a ostatni wymiar musi być 00. Na przykład **+ konto =\[5100\], Departament =\[1100\], MPK =\[01\]** nie jest zawarty w sprawozdaniu, ponieważ ostatni segment jest spoza podanego zakresu. Jeśli wartość segmentu zawiera spacje, należy ująć tę wartość w nawiasach kwadratowych (\[\]). Następujące wartości są prawidłowe dla danego segmentu czterech znaków: **\[234\], \[123 \], \[1 34\]**. Wartości wymiarów powinny być ujęte w nawiasy kwadratowe (\[\]) i raport Projektant dodaje te nawiasy dla Ciebie. Kiedy wiele segmentu wielu lub wiele wymiar zawiera symbol wieloznaczny znaków (? Lub \*), małe i duże końce zakresu całego segmentu wielu lub wiele wymiarów są określane i następnie końcowa i wszystkie wartości między nimi są uwzględniane. Jeśli zakres jest duży, np. cały zakres kont od 40000 do 99999, należy określić prawidłowe konto początkowe i końcowe, jeśli jest to możliwe. **Uwaga:** projektant raportu nie można wybrać konta, wymiary lub pól z systemu Microsoft Dynamics ERP, który zawierać żadnego z następujących znaków zastrzeżonych: &, \*, \[, \], {, lub}. Można dodać znak „&” tylko w przypadku automatycznego tworzenia definicji wierszy przy użyciu okna dialogowego **Wstawianie wierszy z wymiarów**.
...
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| Dodaj dwa konta w pełni kwalifikowane.                                                       | + Rejon =\[000\], konto =\[1205\], dział =\[00\]+ rejon =\[100\], konta =\[1205\], dział =\[00\] |
...
| Dodaj zakres kont w pełni kwalifikowanych.                                                | + Rejon =\[000:100\], konto =\[1205\], Departament =\[00\]                                                   |
| Dodaj zakres wartości segmentów.                                                          | + Konto =\[1200:1205\]                                                                                       |
| Dodaj zakres wartości segmentów z symbolami wieloznacznymi.                         | + Konto =\[120?: 130?\]                                                                                       |
| Odejmij jedno konto w pełni kwalifikowane od innego konta w pełni kwalifikowanego.              | + Rejon =\[000\], konta =\[1205\], dział =\[00\]-rejon =\[100\], konta =\[1205\], Departament =\[00\] |
...
| Odejmij zakres kont w pełni kwalifikowanych.                                           | -Rejon =\[000:100\], konto =\[1200:1205\], Departament =\[00:01\]                                           |
...
Chociaż można modyfikować konta bezpośrednio, można również użyć okna dialogowego **Wymiary**, aby zastosować prawidłowe formatowanie do łącza danych finansowych. Jedną z wartości może zawierać symboli wieloznacznych znaków (? Or \*). Jednak projektant raportu nie można wybrać konta, wymiary lub pól z systemu Microsoft Dynamics ERP, który zawierać żadnego z następujących znaków zastrzeżonych: &, \*, \[, \], {, lub}. **Uwaga:** aby odjąć wartości, trzeba umieścić te wartości w nawiasach. Na przykład, jeśli wprowadzisz **450?-(4509)**, jest on wyświetlany jako **+ konto =\[4509\]-konto =\[450? \]**, i w przypadku polecająca Projektant raportów należy odjąć kwotę dla segmentu konta 4509 od kwoty każdego segmentu konta, który rozpoczyna się od 450.
...
3.  W pierwszym wierszu okna dialogowego **Wymiary** wykonaj następujące kroki:
    ...
    2.  Kliknij dwukrotnie **operatora + /** komórki, a następnie wybierz plus (**+**) lub minus (**-**) operator, który ma zastosowanie do jednej lub wielu wartości wymiaru lub ustawia w wierszu.
...
**Uwaga:** operator ma zastosowanie do wszystkich wymiarów w wierszu.
...
| Pozycja                | Opis                                                                                                                                                                                                                                                                                             |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Pojedynczy lub zakres | W **z** pól, wprowadź nazwę konta lub kliknij **Przeglądaj** przycisk ![Przeglądaj](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Przeglądaj") do przeglądania dla konta. Aby wybrać zakres, wpisz lub przeglądaj, aby wybrać wartość w polu **Do**.                                             |
| Zestaw wartości wymiarów | W polu **Nazwa** wprowadź nazwę zestawu wartości wymiarów. Aby utworzyć, zmodyfikować lub usunąć zestaw, kliknij **Zarządzaj zestawami wartości wymiarów**. **Formuła** pole jest wypełniane przy użyciu formuły z **łącze do wymiarów finansowych** komórki dla tej wartości wymiaru w definicji wiersza. |
...
5.  W oknie dialogowym **Modyfikowanie** modyfikuj wartości formuły, które mają znaleźć się w zestawie. **Uwaga:** jeśli dodajesz nowe konta lub wymiary pamiętaj, aby zmodyfikować istniejące zestawy wartości wymiaru, tak aby uwzględniały zmiany.
