...
> [!Note] 
> Niektóre dokumenty obsługują także nagłówka dokumentu kwot pieniężnych, takie jak opłaty za zamówień i faktur. 
...
-   **Rozdziel kwoty** — Wyświetlanie i modyfikowanie zasad podziału księgowań dla pojedynczego nagłówka lub wiersza dokumentu i wszystkich wierszy podrzędnych, takich jak podatki lub opłaty.
    ...
    -   Wszelkie podrzędne zasady podziału księgowań, takie jak rabat, opłata i podatek, zawsze mają nadrzędną zasadę podziału księgowań. Można wyświetlić relację nadrzędny/podrzędny o **odwołania**&gt;**element nadrzędny informacji**.
    ...
    -   Zasady podziału księgowań dla odchylenia mogą być generowane w scenariuszach uzgadniania, takich jak uzgadnianie między fakturą od dostawcy a zamówieniem zakupu. Można wyświetlić pasujące stosunków między zasady podziału księgowań na **odwołania**&gt;**informacji o dokumencie**.
    -   Przycisk **Popraw** jest dostępny dla dokumentów, które obsługują korekty. **Odpowiednie** tworzy nowe dystrybucje. Po pierwsze dystrybucje są tworzone wycofujące oryginalne dystrybucje. Nie można modyfikować tych dystrybucji. Następny, nowe odpowiednie zasady podziału księgowań są tworzone. Te zasady podziału księgowań mogą być modyfikowane, jeśli oryginalne zasady podziału księgowań mogły być modyfikowane.
    ...
    -   Można wyświetlić aktualny stan księgowania dokumentu w **odwołania**. Stan jest w całym dokumencie i wskazuje, czy dokument jest w toku lub zakończona.
-   ** Zobacz dystrybucje ** — umożliwia wyświetlanie zasad podziału księgowań dla wszystkich wierszy i kwot pieniężnych w dokumencie. Nie można zmodyfikować zasad podziału księgowań w tym widoku.


Aby uzyskać więcej informacji, zobacz [zasady podziału księgowań i zapisy w arkuszu księgi podrzędnej dla faktur niezależnych](accounting-distributions-subledger-journal-entries-vendor-invoices.md).
