...
Projekt jest grupy działań, który jest przeznaczony do świadczenia usług, produktem lub osiągnięcia wyniku. Projekty użycie zasobów i generowania wyników finansowych w postaci przychody lub aktywa.

## <a name="projects-across-industries"></a>Projekty różnych branż
Funkcje zarządzania projektami i księgowania można wykorzystywać w różnych branżach, jak pokazano na poniższej ilustracji. [![Projekty różnych branż](./media/projects-accross-industries.jpg)](./media/projects-accross-industries.jpg) 

W biurze obsługi można użyć biletu do opisania zestawu działań wymaganych do rozwiązania zgłoszonego problemu. Firmy konsultingowe, takich jak zarządzanie lub technicznych konsultacji organizacji lub agencji reklamowych, odnoszą się do ich działalności jako projekty. W dziale marketingu, kampanii reprezentuje zestaw roboczy, który musi zostać dostarczony. W produkcji na podstawie projektu, zlecenie produkcyjne dotyczy różnych prac, które muszą być wykonane w celu wyprodukowania gotowego towaru. Bez względu na nazwę jest używany dla nich, projekty te obejmują zasoby, harmonogramy i koszty i zarządzania projektem i funkcje obsługi kont w usłudze Microsoft Dynamics 365 dla operacji może pomóc w planowanie, wykonanie i analizy tych projektów.
...
Przy uruchamianiu projektu występują następujące procesy. Oferty w ramach projektu można użyć do komunikacji szacowany pracy, wydatki i materiały do odbiorcy. W umowie dotyczącej projektu można rejestrować rozliczeń warunki, ograniczenia i umów. Struktury podziału pracy (SPP) umożliwia planowanie i Szacowanie pracy. Budżety i prognozy można ustawić przypomnienie realizacji projektu. Następująca ilustracja pokazuje strukturę projektu. [![struktury projektu](./media/project-structure1.jpg)](./media/project-structure1.jpg)  
...
Korzystaj z prognozowania projektu, jeśli Twoja organizacja ma perspektywę organizacyjną i koncentruje się przychodach i kosztach pochodzących z określonych transakcji. Korzystaj z budżetowania projektu, jeśli Twoja organizacja bardziej koncentruje się na kwotach finansowych. Każda metoda ma swoje zalety. Aby uzyskać więcej informacji, zobacz [Budżety i prognozy projektu](project-forecasts-budgets.mdhttps:/ax.help.dynamics.com/en/wiki/project-forecasts-and-budgets/).

### <a name="create-projects"></a>Tworzenie projektów

Sześć typów projektów można utworzyć w programie Microsoft Dynamics 365 dla operacji. Każdy typ projektu skonfigurowano inaczej dla kosztów i przychodów. Wybrany typ projektu zależy od przeznaczenia projektu. W poniższej tabeli opisano typowe użycie każdego typu projektu.
...
| Stała cena       | W projektach o stałej cenie faktur składają się z transakcji akonto. Zgodnie z harmonogramem fakturowania, który jest oparty na umowie dotyczącej projektu fakturowania projektu o stałej cenie. Przychód dla projektu o stałej cenie może być obliczany i księgowany przez cały okres trwania projektu przy użyciu metody procentu ukończenia. Można też obliczać i księgować przychód po zakończeniu projektu przy użyciu metody zakończonego kontraktu. Przedsiębiorstwa często korzystają z wartości prac w toku (PWT) do obliczania stopnia ukończenia projektu lub grupy projektów.                                                                                                                                     |
...
> [!NOTE] 
> Transakcje w projektach inwestycyjnych nie są wyświetlane na **Księgowanie kosztów**, **Naliczony przychód**, lub **Tworzenie propozycji faktury** strony.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | | Projekt kosztów | Jak projekty inwestycyjne finansowane projekty są zazwyczaj używane do śledzenia projektów wewnętrznych i tylko godziny, wydatki i elementy mogą być rejestrowane dla nich. Projekty kosztów są zwykle określane na okres krótszy niż projekty inwestycyjne. Dodatkowo, w przeciwieństwie do projektów inwestycyjnych projekty kosztów nie mogą być kapitalizowane na kontach bilansowych. Zamiast tego transakcje projektowe są księgowane tylko na kontach zysku i straty. 
[!NOTE] 
> Transakcje w projektach kosztów są odzwierciedlane **Księgowanie kosztów**, **Naliczony przychód**, lub **Tworzenie propozycji faktury** strony. Ponieważ finansowane projekty są zwykle używane do śledzenia projektów wewnętrznych, one zwykle nie muszą być skojarzone z kontem odbiorcy. Jednak jeśli konfiguracja wymaga, aby dla zamówień zakupu można tworzyć zapotrzebowanie na towary, należy skojarzyć projekt kosztowy z odbiorcą. Jest to spowodowane tym, że zapotrzebowania na towary są zarządzane jako wiersze zamówienia sprzedaży, a system wymaga określenia odbiorcy. Jednak ta konfiguracja nie spowoduje automatycznego tworzenia zapotrzebowania na towary na podstawie zamówienia zakupu. W przypadku projektów kosztów ustawienie **tworzenie zapotrzebowania na towary** jest ignorowane. Jeśli jest potrzebne zapotrzebowanie na towary w projektach kosztowych, możesz utworzyć je ręcznie, o ile odbiorca jest skojarzony z projektem. | | Wewnętrzne | Projekty wewnętrzne są używane do śledzenia kosztów w projekcie, który jest wewnętrznym danej organizacji. Projekty wewnętrzne zapewnia narzędzie do planowania zarządzania zużycie zasobów. **Uwaga:** transakcje w projektach wewnętrznych są odzwierciedlane **Naliczony przychód** lub **Tworzenie propozycji faktury** strony.                                                                                                                                                                                                                                                                       | | Czas | Projekty typu czas są używane do śledzenia czas, który jest skojarzony z działalności Niepłatny i nieprodukcyjnych, takich jak projekt do śledzenia zwolnienie lekarskie dla pracowników. Transakcje w projektach typu czas nie są księgowane w księdze. Zamiast tego są uwzględniane w raportach wykorzystania pracownika. Tylko transakcje godzinowe mogą być rejestrowane w projektach czasowych. Arkusz godzin lub karta czasu pracy służy do rejestrowania tych godzin w projekcie. Po zarejestrowaniu godziny są wyświetlane jako transakcje projektu, ale nie ma odpowiadającego transakcje na załączniku.

> [!NOTE] 
> Transakcje w projektach typu czas są odzwierciedlane **Księgowanie kosztów**, **Naliczony przychód**, lub **Tworzenie propozycji faktury** strony.                                                                                                                                                                                       |
...
Oto niektóre ze sposobów, których można używać funkcji planowania zasobów:
...
Istnieją dwa sposoby zarządzania i kontroli projekty: projekt prognoz oraz projektów budżetów. Korzystaj z prognozowania projektu, jeśli Twoja organizacja ma perspektywę organizacyjną i koncentruje się przychodach i kosztach pochodzących z określonych transakcji. Korzystaj z budżetowania projektu, jeśli Twoja organizacja bardziej koncentruje się na kwotach finansowych.

Aby uzyskać więcej informacji, zobacz [Budżety i prognozy projektu](project-forecasts-budgets.mdhttps:/ax.help.dynamics.com/en/wiki/project-forecasts-and-budgets/).
...
-   Aby zamówić towary z magazynu firmy, trzeba utworzyć zamówienie jako zapotrzebowanie na towary. Jeśli używasz **przedmiotu wymagania** stronę, można skonfigurować wymóg, aby otrzymywać elementów jako dostaw częściowych. W związku z tym można odłożyć zużycie ilości towarów, dopóki elementy są wymagane.
-   W przypadku zamawiania towarów u dostawcy zewnętrznego trzeba utworzyć zamówienie zakupu na stronie **Zamówienie zakupu**.

> [!NOTE] 
> Nie można anulować dostawy dla zamówienia sprzedaży związanych z projektem, jeśli już zostały oznaczone elementy do pakowania. 
...
Microsoft Dynamics 365 dla operacji automatycznie nie maleje prognozowanych oszacowań. Dlatego warto zachować pierwotny model prognozy w projekcie o stałej cenie, aby mieć podstawę do porównania, gdy projekt zostanie zakończony. 
...
-   **Data faktury:** 31 marca 2012. Ta data jest określana w oparciu o częstotliwość faktur. W tym przykładzie ustawia się częstotliwość faktury na bieżący miesiąc. W związku z tym wszystkie, które są księgowane w marcu są fakturowane w ostatnim dniu miesiąca.
-   **Data ukończenia:** 14 kwietnia 2012. Ta data jest określana na podstawie warunków płatności, które zostały ustawione dla projektu. W tym przykładzie zaznaczono warunki płatności 14 dni. W związku z tym 14 dni dodaje się do daty faktury, by otrzymać termin realizacji 14 kwietnia 2012.
-   **Oczekiwana data płatności za sprzedaż:** 27 kwietnia 2012. Data ta jest obliczana przez dodanie liczby dni w **ogólne dni buforowe** w **Zarządzanie projektami i ich księgowanie parametry** stronę do liczby dni w **indywidualne dni buforowe** w **umowy dotyczące projektu** strona, a następnie dodanie całkowitej liczby dni w **Data ukończenia** pole. Na przykład wprowadzono **3** w polu **ogólne dni buforowe** i **10** w polu **indywidualne dni buforowe**. W związku z tym 13 dni dodaje się do terminu, oczekiwany termin płatności 27 kwietnia 2012 r.
...
-   **Data ukończenia:** 16 marca 2012. Warunki płatności są ustawione na 14 dni.
-   **Oczekiwana data płatności za sprzedaż:** 29 marca 2012. Obliczenie obejmuje trzy dni buforowe ogólne i 10 indywidualne dni buforowe.
...
> [!NOTE] 
> Termin dla zamówienia zakupu opiera się na transakcji dostawcy podczas tworzenia zamówienia zakupu dla projektu. Termin nie jest określany przez żadne ustawienia domyślne. 
...
> [!NOTE] 
> Użycie **Kontrola kosztów** stronę, aby wyświetlić bieżący stan kosztów projektu, należy użyć modeli prognoz, które zostały wybrane dla budżetu oryginalnych i pozostałych. Jeśli podczas obliczania kosztów wybierzesz inne modele prognozy, wyniki obliczeń nie będą prawidłowe.
