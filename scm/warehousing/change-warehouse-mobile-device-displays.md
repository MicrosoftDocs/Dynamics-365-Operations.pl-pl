...
&lt;Nazwa formantu&gt;(&lt;nazwa klucza&gt;) =&lt;kod klawisza&gt;;

Poniżej znajdują się wyjaśnienia dotyczące części składni:

-   **&lt;Nazwa formantu&gt;** — nazwa formantu (na przykład przycisk), który jest renderowany w języku HTML.
-   **(&lt;nazwa klucza&gt;) ** – Nazwę klawisz klawiatury, którego tworzysz skrótem.
-   **&lt;Kod klawisza&gt;** — kod znaku numerycznego klucz do użycia jako klawisz skrótu.
...
Request.UserHostAddress=&lt;adres hosta użytkownika&gt;| HostName =&lt;nazwa hosta użytkownika&gt;| Request.UserAgent=&lt;agenta użytkownika&gt;

Poniżej znajdują się wyjaśnienia dotyczące części wyrażenia:

-   **&lt;adres hosta użytkownika&gt;** -A .NET wyrażenia regularnego, który pasuje do adresu IP żądającego.
-   **&lt;Nazwa hosta użytkownika&gt;** -A .NET wyrażenia regularnego, który pasuje do nazwy sieci żądającego.
-   **&lt;agent użytkownika&gt;** -A .NET wyrażenia regularnego, który odpowiada identyfikator przeglądarki, która używa żądającego.
...
Można rozszerzyć listę zaakceptowanych formatów dat dla każdej instalacji. Na funkcja może być przydatna, jeśli chcesz podać format, który ułatwi pracownikowi wprowadzenie daty na urządzeniu przenośnym. Domyślny format zależy domyślnego języka użytkownika, który określono w polu **języka** na stronie **opcji użytkownika**. (Tej samej stronie umożliwia również kojarzenie pracownika z pracy określonego magazynu użytkownika.) **Uwaga:** Portal urządzeń Mobile magazynu nie korzysta z ustawień z **Data, czas i format liczby** w **preferencji języka i regionu** strony. Aby zmienić format daty, należy zapoznać się z wyrażeniami regularnymi w programie .NET Framework. Aby uzyskać więcej informacji, zobacz temat [Wyrażenia regularne systemu .NET Framework](http://go.microsoft.com/fwlink/?LinkId=391260). Aby zdefiniować formaty daty, Edytuj plik Dates.ini, który znajduje się w treści\\ustawienia\\Dates.ini na serwerze portalu urządzeń Mobile magazynu. Ten plik zawiera wyrażenia regularne .NET do określenia formatu daty. Wyrażenie regularne musi zawierać podrzędne wyrażenia tworzące grupy nazw dla dnia, miesiąca i roku (DDMMYY), jak to przedstawiono w poniższym przykładzie:
