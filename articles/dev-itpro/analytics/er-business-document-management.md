...
**Zarządzanie dokumentami biznesowymi** jest oparte na górnej części struktury ER i umożliwia użytkownikom biznesowym edytowanie szablonów dokumentów biznesowych przy użyciu usługi Microsoft Office 365 lub odpowiedniej aplikacji klasycznych pakietu Microsoft Office. Edycja dokumentów może obejmować zmianę projektów dokumentów biznesowych i dodanie symboli zastępczych dla dodatkowych danych z poziomu Dynamics 365 for Finance and Operations bez zmian kodu źródłowego i nowych wdrożeń. Do aktualizacji szablonów dokumentów biznesowych nie jest wymagana znajomość struktury ER.
...
Aby używać funkcji zarządzania dokumentami biznesowymi do edytowania szablonów w formatach programu Excel lub Word poprzez użycie aplikacji klasycznych pakietu Microsoft Office, należy zainstalować pakiet Microsoft Office  2010 lub nowszy. Jest to obsługiwane w ramach wdrażania Finance and Operations w chmurze oraz lokalnie
...
Należy zaimportować konfiguracje ER zawierające szablony dokumentów biznesowych do bieżącego wystąpienia Finance and Operations. Aby wykonać tę procedurę, należy pobrać i lokalnie zapisać poniższe pliki.
...
1. Zaloguj się do Finance and Operations jako użytkownik z dostępem do strony **Parametry dokumentu biznesowego**.
...
1. Zaloguj się do Finance and Operations jako użytkownik z dostępem do strony **Konfigurator uprawnień dostępu**.
...
> [!NOTE]
> Reguły **Uprawnienia dostępu wg konfiguracji** są przechowywane przy użyciu unikatowego identyfikatora konfiguracji formatu ER. Oznacza to, że te reguły nie zostaną usunięte po usunięciu konfiguracji ER odwołującej się do tych reguł. Po zaimportowaniu usuniętych konfiguracji z powrotem do tego wystąpienia Finance and Operations te reguły będą odnosiły się do nich na nowo. Nie ma potrzeby ponownego konfigurowania reguł po ponownym zaimportowaniu usuniętych konfiguracji.
...
1. Zaloguj się do Finance and Operations jako użytkownik z dostępem do strony obszaru roboczego zarządzania dokumentu biznesowego.
...
Zostanieotwarta strona **edytora szablonów BDM** w Finance and Operations. Wybrany szablon będzie dostępny do edycji w trybie online za pomocą Office 365.
...
1. W module Finance and Operations zmień firmę na **USMF**.
...
2. Jeśli zostanie wybrana **OK** w celu potwierdzenia publikowania, wersja robocza raportu **kopii FTI raportu odbiorcy (GER)** formatu ER, która zawiera zmodyfikowany szablon, zostanie oznaczona jako zakończona. Zmodyfikowany szablon staje się dostępny dla innych użytkowników Finance and Operations. Wersje ukończone tego formatu ER zachowają tylko ostatnią aktywną korektę szablonu. Inne zmiany zostaną usunięte. Wybierz **Anuluj**, aby poznać inne opcje.
...
Jest to znany błąd dotyczący Office 365 przekierowywania z powrotem do Finance and Operations. Dzieje się tak po pierwszym zalogowaniu Office 365. W celu obejścia tego problemu należy wybrać **Wstecz** w przeglądarce, aby przejść z powrotem do Finance and Operations.
...
Najprawdopodobniej zalogowano się do bieżącego wystąpienia finansów i operacji w domenie Azure AD, które różnią się od domeny Azure AD, która została użyta do wdrożenia tego wystąpienia Finance and Operations. Ponieważ usługa SharePoint, która jest używana do przechowywania szablonów do udostępniania ich do edycji za pomocą aplikacji klasycznych pakietu Office, należy do tej samej domeny co Finance and Operations nie ma żadnych uprawnień dostępu do usługi SharePoint. Aby rozwiązać ten problem, zaloguj się do bieżącego wystąpienia Finance and Operations, używając poświadczeń użytkownika z poprawną domeną Azure AD.
