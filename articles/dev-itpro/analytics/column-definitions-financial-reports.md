...
Projektant raportów może automatycznie wygenerować nagłówki kolumn na podstawie kodów autotekstu. Kody autotekstu są wartościami zmiennymi, które są aktualizowane po każdym wygenerowaniu raportu. Każdy nagłówek kolumny może zawierać te kody, aby określać informacje raportu, które mogą być różne, np. daty czy numery okresów. W związku z tym jedna definicja kolumny może być używana w odniesieniu do wielu definicji raportu, okresów czasu i drzew raportowania. Kody autotekstu są zależne od informacji kalendarza z wierszy szczegółów definicji kolumny, dlatego są obsługiwane tylko w kolumnach <strong>OBL</strong>, <strong>WF</strong> i <strong>ARK</strong>. Sposób wyświetlania kodu autotekstu w nagłówku kolumny ma wpływ na sposób wyświetlania informacji w raporcie. W oknie dialogowym <strong>Nagłówek kolumny</strong> kody autotekstu zawierają wielkie i małe litery. Dlatego w także w raporcie tekst jest wyświetlany przy użyciu małych i dużych liter. Na przykład w standardowym roku kalendarzowy kod <strong>@CalMonthLong</strong> rozwiązuje cyfrę <strong>7</strong> do miesiąca <strong>Lipiec</strong>. Jeśli nazwa miesiąca powinna być pisana dużymi literami (na przykład <strong>LIPIEC</strong>), wprowadź kod autotekstu dużymi literami w polu <strong>Tekst nagłówka kolumny</strong>. Na przykład wpisz <strong>@CALMONTHLONG</strong>. Można łączyć ze sobą kody i tekst. Na przykład można wprowadzić następujący tekst nagłówka: <strong>Okres @FiscalPeriod-@FiscalYear od @StartDate do @EndDate</strong>. Generowany nagłówek raportu będzie wyglądał mniej więcej tak: <strong>Okres 1-02 od 01-01-2002 do 31-01-2002</strong>. <strong>Uwaga:</strong> Format niektórych tekstów, np. długich dat, zależy od ustawień regionalnych na serwerze programu Finance and Operations. Aby zmienić te ustawienia, kliknij przycisk <strong>Start</strong>, kliknij <strong>Panel sterowania</strong>, a następnie kliknij <strong>Region i język</strong>. W poniższej tabeli pokazano dostępne opcje autotekstu dla nagłówków kolumn.
...
|----------------------------------------------|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|         Waluta naturalna/pierwotna         | <strong>Jen</strong> |                                                                   <strong>Y6,000</strong> — wynik pokazuje tylko transakcje wprowadzone w jenach.                                                                   |
| Waluta funkcjonalna na podstawie informacji o firmie | <strong>Jen</strong> |          <strong>$60*</strong> — wynik pokazuje tylko transakcje wprowadzone w jenach i wyświetla je w dolarach. <strong>Uwaga:</strong> Kurs przeliczeniowy to około 100 jenów za 1 USD.           |
| Waluta funkcjonalna z informacji o firmie |        Pusty         | <strong>2310 USD\*\</strong>* — wynik pokazuje wszystkie dane w walucie funkcjonalnej określonej w informacjach o firmie. <strong>Uwaga:</strong> Ta kwota jest sumą wszystkich transakcji w walucie funkcjonalnej. |
|         Waluta naturalna/pierwotna         |        Puste         |                                                       <strong>$2250</strong> — wynik pokazuje wszystkie kwoty w walucie, w której przeprowadzono transakcję.                                                        |

### <a name="calculation-column-in-a-column-definition"></a>Kolumna obliczania w definicji kolumny

Typ kolumny **CALC** w definicji kolumn pomaga w wykonywaniu złożonych obliczeń w komórce <strong>Formuła</strong> i może zawierać operatory <strong>+</strong>, <strong>-</strong>, <strong>\</strong> i **/* oraz instrukcje <strong>IF/THEN/ELSE</strong>. Kolumna obliczeń może również odnosić się do dowolnej innej kolumny, a nawet do kolejnych kolumn. Ponadto kolumna obliczania może również zawierać rok obrachunkowy i okres do obsługi nagłówków dla kolumny. Formuła obliczania może zawierać do 1024 znaków. Aby wyrazić wynik obliczeń jako wartość procentową, należy użyć specjalnego zastąpienia formatu. <strong>Uwaga:</strong> Wyniki formuł obliczania nie zawierają wartości z niedrukowanych zakresów kolumn. Na przykład formuła <strong>A:D</strong> spowoduje drukowanie <strong>0</strong> (zero), a formuła <strong>A+B+C</strong> dla niedrukowanych wartości spowoduje obliczenie wartości.
