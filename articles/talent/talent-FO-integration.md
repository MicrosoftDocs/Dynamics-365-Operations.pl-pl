...
W tym temacie zawarto odpowiedzi na podstawowe pytania dotyczące sposobu synchronizowania danych po zintegrowaniu programu Dynamics 365 Talent z programem Dynamics 365 Finance.
...
## <a name="can-i-map-financial-dimensions-to-transfer-between-talent-and-finance"></a>Czy można mapować wymiary finansowe do przeniesienia między programami Talent i Finance?
...
Nie. Integracja jest obecnie jednokierunkowa (z rozwiązania Talent do rozwiązania Finance and Operations). Dostępny jest jednak domyślny szablon do wysyłania danych z rozwiązania Talent do rozwiązania Finance.
...
## <a name="i-just-created-new-finance-and-talent-environments-and-im-getting-the-error-the-data-value-violates-integrity-constraints-why"></a>Po utworzeniu nowego środowiska rozwiązań Finance i Talent pojawia się błąd „Wartość danych narusza ograniczenie integralności”. Dlaczego?
...
Sekwencja numerów dla **Identyfikatora lokalizacji** korzysta z takiego samego wzorca zarówno w Talent jaki i w Finance. Sekwencja numerów musi być unikatowa po obu stronach, żeby nie było żadnych konfliktów adresów podczas integrowania danych z Common Data Service do Finance and Operations.

Podczas wprowadzania środowiska Talent należy sprawdzić, czy sekwencja numerów nie jest taka sama w Talent i Finance. Sprawdź, czy wszystkie sekwencje numerów nie są identyczne tam gdzie dane mogą być obsługiwane przez oba systemy.
...
Upewnij się, że podczas tworzenia połączenia została wybrana opcja Dynamics 365 Finance i Common Data Service.
...
Upewnij się, że mapujesz prawidłowe firmy. Synchronizowanie firmy nie jest częścią szablonu domyślnego, więc oczekuje się, że każda firma, która znajduje się w Talent i Common Data Service, znajduje się również w Finance.
