---
title: Tworzenie zespołu rekrutacyjnego za pomocą Dynamics 365 Talent - Onboard
description: W tym temacie wyjaśniono, jak używać aplikacji Microsoft Dynamics 365 Talent - Onboard firmy Microsoft do tworzenia zespołów wdrażania do pracy.
author: andreabichsel
manager: ''
ms.date: 05/02/2019
ms.topic: article
ms.prod: ''
ms.service: dynamics-365-talent
ms.technology: ''
ms.search.form: HcmCourseType, HcmCourseTypeGroup, HRMCourseTable
audience: Application User
ms.reviewer: anbichse
ms.search.scope: Core, Operations, Talent
ms.search.region: Global
ms.author: anbichse
ms.search.validFrom: 2019-05-06
ms.dyn365.ops.version: Talent
ms.openlocfilehash: b22bf5fb5b2a2756684304d1cb26c0f8a7419c00
ms.sourcegitcommit: 434dd21450bddcd891aba0555b9853d9ba0afb6f
ms.translationtype: HT
ms.contentlocale: pl-PL
ms.lasthandoff: 09/23/2019
ms.locfileid: "2010828"
---
# <a name="create-a-hiring-team-by-using-onboard"></a>Tworzenie zespołu rekrutacyjnego za pomocą Onboard

[!include [banner](includes/banner.md)]

W Microsoft Dynamics 365 Talent: Onboard można tworzyć zespoły rekrutacyjne. Następnie można przypisać przewodniki i szablony wdrażania do pracy do każdego zespołu.

## <a name="create-a-hiring-team"></a>Tworzenie zespołu rekrutacyjnego

1. W menu po lewej stronie wybierz **Zespoły**.
2. W obszarze **Zespoły** wybierz kafelek **Dodaj** (znak plus \[**+**\]).
3. W oknie dialogowym **Utwórz nowy zespół**, pod **Nazwa zespołu** wprowadź nazwę dla zespołu rekrutacyjnego.

    ![[Tworzenie nowego zespołu w Onboard](./media/onboard-create-team.png)](./media/onboard-create-team.png)

4. W obszarze **Wybierz członków zespołu**, wprowadź nazwę lub adres e-mail każdego członka zespołu.

    Aby usunąć członków zespołu, wybierz **X**, który pojawia się obok ich nazwy w polu.

5. Aby wysłać wiadomość e-mail do nowych członków zespołu, włącz opcję **Wyślij wiadomość e-mail do nowych** członków.
6. Wybierz opcję **Utwórz**.
7. Jeśli wskazano, że chcesz wysłać wiadomości e-mail do nowych członków zespołu, edytuj wiadomość e-mail w następnym oknie dialogowym, a następnie wybierz pozycję **Gotowe**.

## <a name="assign-onboarding-guides-to-a-hiring-team"></a>Przypisywanie przewodników wdrażania do pracy do zespołu rekruterów

1. W menu po lewej stronie wybierz **Zespoły**.
2. Wybierz zespół.
3. Na karcie **Przewodniki** wybierz przycisk **Dodaj przewodniki**.

    ![[Dodawanie poradników dołączania do zespołu](./media/onboard-add-guides-to-team.png)](./media/onboard-add-guides-to-team.png)

4. Zaznacz pole wyboru dla każdego przewodnika wdrażania do pracy, który chcesz przypisać do zespołu, a następnie wybierz **Dodaj**.

    ![[Wybieranie poradników wdrażania do pracy do dołączenia do zespołu](./media/onboard-select-guides.png)](./media/onboard-select-guides.png)

## <a name="assign-onboarding-templates-to-a-hiring-team"></a>Przypisywanie szablonów przewodników wdrażania do pracy do zespołu rekrutacyjnego

1. W menu po lewej stronie wybierz **Zespoły**.
2. Wybierz zespół.
3. Na karcie **Szablony** wybierz przycisk **Dodaj szablony**.

    ![[Dodawanie szablonów do zespołu](./media/onboard-add-templates-to-team.png)](./media/onboard-add-templates-to-team.png)

4. Zaznacz pole wyboru dla każdego szablonu, który chcesz przypisać do zespołu, a następnie wybierz **Dodaj**.

    ![[Wybieranie szablonów do dołączenia do zespołu](./media/onboard-select-templates.png)](./media/onboard-select-templates.png)

### <a name="see-also"></a>Informacje dodatkowe

- [Wypróbuj lub kup aplikację Onboard](https://dynamics.microsoft.com/talent/onboard/)
- [Co nowego](./whats-new.md)
- [Informacje o wersji](https://docs.microsoft.com/business-applications-release-notes/index)
- [Uzyskiwanie pomocy technicznej](./talent-support.md)
