...
2. Na karcie **Klucze konfiguracji** rozwiń węzeł **Handel detaliczny**, a następnie wybierz pole wyboru **Zarządzanie zamówieniami rozdzielonymi**.
3. Przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Ustawienia \> Parametry DOM**.
4. Na karcie **Ogólne** ustaw następujące wartości:
    ...
    - **Potwierdź użycie Map Bing dla DOM** — ustaw wartość **Tak** dla tej opcji.
        ...
        > [!NOTE]
        > Dla tej opcji możesz ustawić wartość **Tak**, tylko jeśli ustawiono wartość **Tak** dla opcji **Włącz Mapy Bing** na karcie **Mapy Bing** strony **Wspólne parametry sieci sprzedaży** (**Handel detaliczny \> Ustawienia centrali \> Parametry \> Wspólne parametry sieci sprzedaży**) oraz jeśli wprowadzono prawidłowy klucz w polu **Klucz Map Bing**.
...
5. Na karcie **Zmienna** ustaw następujące wartości:
    ...
    - **Typ zmiennej** — wybierz wartości. W aplikacji Retail zwalniane są dwa typy zmiennych: **Zmienna produkcji** i **Zmienna uproszczona**. W przypadku wszystkich maszyn z uruchomioną funkcją DOM (tj. wszystkich serwerów, które są częścią grupy DOMBatch) wybierz typ **Zmienna produkcji**. Zmienna produkcji wymaga specjalnego klucza licencji, który domyślnie jest licencjonowany i wdrażany w środowiskach produkcyjnych. W przypadku środowisk nieprodukcyjnych wdróż ten klucz licencji ręcznie. Aby ręcznie wdrożyć klucz licencji, wykonaj następujące czynności:
        ...
        4. Przejdź do klienta aplikacji Retail Headquarters, a następnie otwórz stronę **Parametry DOM**. Na karcie **Zmienna** w polu **Typ zmiennej** zaznacz opcję **Zmienna produkcji** i upewnij się, że nie są wyświetlane żadne komunikaty o błędach.
...
6. Przejdź wstecz do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Ustawienia \> Parametry DOM**.
7. Na karcie **Sekwencje numerów** przypisz wymagane sekwencje numerów do różnych jednostek DOM.
...
8. Funkcja DOM obsługuje definicje różnych typów reguł DOM. Organizacje mogą konfigurować wiele reguł w zależności od potrzeb biznesowych. Reguły DOM możesz zdefiniować dla grupy lokalizacji lub poszczególnych lokalizacji, a także dla określonej kategorii produktów, produktu lub wariantu. Aby utworzyć grupowanie lokalizacji, które muszą być używane w regułach DOM, wykonaj następujące czynności:
    ...
    1. Przejdź do opcji **Handel detaliczny \> Konfiguracja kanału \> Grupy realizacji**.
...
9. Aby zdefiniować reguły, przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Ustawienia \> Zarządzaj regułami**. Obecnie obsługiwane są następujące reguły DOM:
...
10. Profile realizacji są używane do grupowania kolekcji reguł, firm, źródeł zamówień sprzedaży i metod dostawy. Każda sesja DOM odnosi się do określonego profilu realizacji. W ten sposób organizacje mogą definiować i uruchamiać zestawy reguł dla zbioru firm w przypadku zamówień, które mają określone źródła zamówień sprzedaży i metody dostawy. W związku z tym, jeśli mają być uruchamiane różne zestawy reguł dla różnych źródeł zamówień sprzedaży lub metod dostawy, możesz odpowiednio zdefiniować profile realizacji. Aby skonfigurować profile realizacji, wykonaj następujące czynności:  
    ...
    1. Przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Ustawienia \> Profile realizacji**.
...
Funkcja DOM zostanie uruchomiona tylko w ramach zadania wsadowego. Aby skonfigurować zadanie wsadowe dla sesji DOM, wykonaj poniższe czynności.

1. Przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Przetwarzanie wsadowe \> Ustawienia zadania procesora DOM**.
...
- Wiersze zamówienia spełniające kryteria określone dla źródeł zamówień sprzedaży, metod dostawy i firmy zgodnie z definicją podaną w profilu DOM, a także wiersze spełniające którekolwiek z tych kryteriów:
    ...
    - Zostały utworzone w kanałach sprzedaży detalicznej.
...
1. Przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Zarządzanie zamówieniami rozdzielonymi**.
...
Poniżej opisano ustawienia w wierszu zamówienia. Aby otworzyć wiersz zamówienia, przejdź do opcji **Handel detaliczny \> Odbiorcy \> Wszystkie zamówienia sprzedaży**.
...
1. Przejdź do opcji **Handel detaliczny \> Zarządzanie zamówieniami rozdzielonymi \> Przetwarzanie wsadowe \> Ustawienia zadania usuwania danych realizacji DOM**. 
