...
ms.openlocfilehash: be1f07db35cfdc3cc145e0ea7251ef14abb9cf38f8a96b4f626003bb79b85383
ms.sourcegitcommit: 42fe9790ddf0bdad911544deaa82123a396712fb
ms.translationtype: HT
ms.contentlocale: pl-PL
ms.lasthandoff: 08/05/2021
ms.locfileid: "6733211"
---
...
3. Skopiuj plik zestawu do folderu rozszerzenia CRT:
    ...
    - **Commerce Scale Unit:** skopiuj zestaw do folderu **\\bin\\ext** w lokalizacji Microsoft Internet Information Services (IIS) witryny Commerce Scale Unit.
...
4. Znajdź plik konfiguracji rozszerzeń dla CRT:
    ...
    - **Commerce Scale Unit:** plik nosi nazwę **commerceruntime.ext.config** i jest w folderze bin\\ext w lokalizacji IIS witryny Commerce Scale Unit.
...
Aby włączyć proces rejestracji, wykonaj następujące kroki do skonfigurowania Headquarters. Aby uzyskać więcej informacji, zobacz [Konfigurowanie procesu rejestracji fiskalnej](setting-up-fiscal-integration-for-retail-channel.md#set-up-a-fiscal-registration-process).

1. Wybierz kolejno **Handel detaliczny i inny \> Konfigurowanie kanału \> Integracja fiskalna \> Łączniki fiskalne**. Zaimportuj konfigurację z **RetailSdk\\SampleExtensions\\HardwareStation\\Extension.Posnet.ThermalDeviceSample\\Configuration\\ConnectorConnectorPosnetThermalFVEJ.xml**.
2. Wybierz kolejno **Handel detaliczny i inny \> Ustawienia kanału \> Integracja fiskalna \> Dostawcy dokumentów fiskalnych**. Zaimportuj konfigurację z **RetailSdk\\SampleExtensions\\CommerceRuntime\\Extension.DocumentProvider.PosnetSample\\Configuration\\DocumentProviderPosnetSample.xml**.
3. Wybierz kolejno **Handel detaliczny i inny \> Konfigurowanie kanału \> Integracja fiskalna \> Profile techniczne łącznika**. Utwórz nowy profil i wybierz łącznik załadowany w poprzednim kroku. Zaktualizuj ustawienia połączenia, jeśli wymagana jest aktualizacja.
4. Wybierz kolejno **Handel detaliczny i inny \> Konfigurowanie kanału \> Integracja fiskalna \> Profile funkcjonalne łącznika**. Utwórz nowy profil i wybierz łącznik i dostawcę dokumentów załadowane w poprzednich krokach. Zaktualizuj ustawienia mapowania danych, jeśli aktualizacja jest wymagana.
5. Wybierz kolejno **Handel detaliczny i inny \> Konfigurowanie kanału \> Integracja fiskalna \> Grupa funkcjonalna łącznika**. Utwórz nową grupę i wybierz profil funkcji łącznika z poprzedniego kroku.
6. Wybierz kolejno **Handel detaliczny i inny \> Konfigurowanie kanału \> Integracja fiskalna \> Proces rejestracji**. Utwórz nowy proces i wybierz grupę funkcji łącznika z poprzedniego kroku.
7. Wybierz kolejno opcje **Handel detaliczny i inny \> Ustawienia kanału \> Ustawienia punktu sprzedaży \> Profile punktów sprzedaży \> Profile funkcji**. Otwórz profil funkcji, który jest połączony z magazynem, w którym należy aktywować proces rejestracji. Na skróconej karcie **Proces rejestracji fiskalnej** wybierz proces rejestracji, który został utworzony wcześniej.
...
9. Otwórz harmonogram dystrybucji (**Handel detaliczny i inny \> Składniki IT w handlu detalicznym i innym \> Harmonogram dystrybucji**) i wybierz zadania **1070** i **1090** do przesyłania danych do bazy danych kanału.
...
Program obsługi jest dziedziczony z interfejsu **INamedRequestHandler**. Metoda **HandlerName** odpowiada za zwrócenie nazwa programu obsługi. Nazwa programu obsługi powinna odpowiadać nazwie dostawcy dokumentów łącznika określonej w Headquarters.
...
Plik konfiguracyjny znajduje się w folderze **Konfiguracja** projektu rozszerzenia. Ten plik służy do obsługi konfiguracji ustawień dostawcy dokumentu z Headquarters. Format pliku jest zgodny z wymaganiami konfiguracji integracji fiskalnej. Dodano następujące ustawienia:
...
Program obsługi jest dziedziczony z interfejsu **INamedRequestHandler**. Metoda **HandlerName** odpowiada za zwrócenie nazwa programu obsługi. Nazwa programu obsługi powinna odpowiadać nazwie łącznika fiskalnego określonej w Headquarters.
...
Plik konfiguracyjny znajduje się w folderze **Konfiguracja** projektu rozszerzenia. Ten plik służy do obsługi konfiguracji ustawień łącznika z Headquarters. Format pliku jest zgodny z wymaganiami konfiguracji integracji fiskalnej. Dodano następujące ustawienia:
