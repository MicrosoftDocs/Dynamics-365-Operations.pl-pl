...
description: Jak zainicjować firmę Dataverse lub inne dane aplikacji Dynamics 365 z informacjami o firmie przed włączeniem połączenia z podwójnym zapisywaniem.
...
ms.locfileid: "4683778"
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="ca700-103">Bootstrap z danymi firmy — często zadawane pytania</span><span class="sxs-lookup"><span data-stu-id="ca700-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [rename-banner](~/includes/cc-data-platform-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="ca700-104">Dlaczego potrzebuję inicjowania firmy?</span><span class="sxs-lookup"><span data-stu-id="ca700-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="ca700-105">Być może istnieje już istniejące Dataverse lub inne wystąpienie aplikacji Dynamics 365 z danymi biznesowymi, a użytkownik chce włączyć dla niego połączenie podwójnego odpisu.</span><span class="sxs-lookup"><span data-stu-id="ca700-105">You might have an existing Dataverse or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="ca700-106">W tym przypadku, aby zainicjować firmę Dataverse lub inne dane aplikacji Dynamics 365 z informacjami o firmie przed włączeniem połączenia z podwójnym zapisywaniem.</span><span class="sxs-lookup"><span data-stu-id="ca700-106">In this case, you need to bootstrap Dataverse or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="ca700-107">Kiedy należy używać inicjowania firmy?</span><span class="sxs-lookup"><span data-stu-id="ca700-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="ca700-108">Przed włączeniem map tabeli z podwójnym zapisywaniem (w kroku #5) należy wykonać ładowanie wstępne.</span><span class="sxs-lookup"><span data-stu-id="ca700-108">You should use bootstrapping before enabling dual-write table maps (during step #5).</span></span>  
1. <span data-ttu-id="ca700-109">Aby skonfigurować połączenie podwójnego zapisywania między wystąpieniami aplikacji Finance and Operations oraz z Dataverse lub z inną aplikacją Dynamics 365, zaloguj się do aplikacji Finance and Operations jako administrator.</span><span class="sxs-lookup"><span data-stu-id="ca700-109">To setup the dual-write connection between instances of your Finance and Operations app and the Dataverse or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="ca700-110">Przejdź do modułu **zarządzania danymi** i kliknij przycisk **podwójnego zapisywania**.</span><span class="sxs-lookup"><span data-stu-id="ca700-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="ca700-111">Spowoduje to uruchomienie **integratora danych**.</span><span class="sxs-lookup"><span data-stu-id="ca700-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="ca700-112">Utwórz połączenie podwójnego zapisywania dla co najmniej jednej firmy.</span><span class="sxs-lookup"><span data-stu-id="ca700-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="ca700-113">![Utwórz połączenie podwójnego zapisywania](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="ca700-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="ca700-114">Włącz mapowanie tabeli **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="ca700-114">Enable the **Cdm_companies** table map.</span></span> <span data-ttu-id="ca700-115">Spowoduje to zsynchronizowanie firm z aplikacji Finance and Operations do Dataverse.</span><span class="sxs-lookup"><span data-stu-id="ca700-115">This synchronizes companies from the Finance and Operations app to Dataverse.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="ca700-116">![Włącz mapowanie tabeli](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="ca700-116">![Enable the table map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="ca700-117">Należy uruchomić przykładowy kod inicjujący w wystąpieniu Dataverse lub innym wystąpieniu aplikacji Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="ca700-117">Run the sample bootstrapping code on the Dataverse or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="ca700-118">Po zakończeniu inicjowania firmy i gdy system jest gotowy do synchronizacji na żywo, włącz mapowania tabeli.</span><span class="sxs-lookup"><span data-stu-id="ca700-118">When the bootstrapping is done and the system is ready for the live sync, enable the table maps.</span></span>  

    <span data-ttu-id="ca700-119">Włączenie mapowań tabeli powoduje wyzwolenie początkowej synchronizacji danych dla włączonych mapowań tabeli.</span><span class="sxs-lookup"><span data-stu-id="ca700-119">Enabling the table maps triggers the initial data sync for the enabled table maps.</span></span> <span data-ttu-id="ca700-120">Dane odpowiadające firmom wybranym w połączeniu podwójnego zapisywania są synchronizowane między aplikacją Finance and Operations i Dataverse.</span><span class="sxs-lookup"><span data-stu-id="ca700-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Dataverse.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="ca700-121">Jak mogę wykorzystać przykładowy kod?</span><span class="sxs-lookup"><span data-stu-id="ca700-121">How to I use the code sample?</span></span>
<span data-ttu-id="ca700-122">Przykładowym kodem jest aplikacja C#, które można ładować w Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="ca700-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="ca700-123">Powoduje on zależności pakietów NuGet w zestawie SDK Dataverse, który można odświeżyć za pomocą narzędzia standardowego Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="ca700-123">It takes NuGet package dependencies on the Dataverse SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="ca700-124">Po rozpakować się i otwarciu rozwiązania w Visual Studio i odwróceniu NuGet, należy wyszukać w kodzie **DO ZROBIENIA**.</span><span class="sxs-lookup"><span data-stu-id="ca700-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="ca700-125">Każda decyzja konieczna do zapoznania się z informacjami o firmie, którą należy wykonać, jest zapisywana jako **DO ZROBIENIA** z przykładowym kodem dla implementacji kanonicznej.</span><span class="sxs-lookup"><span data-stu-id="ca700-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="ca700-126">Przykładowy kod pokazuje tylko jeden z wielu sposobów kategoryzowania wierszy jednostek według firm.</span><span class="sxs-lookup"><span data-stu-id="ca700-126">The sample code shows only one of many ways you might categorize entity rows by company.</span></span> <span data-ttu-id="ca700-127">Zmieniając logikę w sekcjach **DO ZROBIENIA**, można utworzyć kategoryzację niestandardową.</span><span class="sxs-lookup"><span data-stu-id="ca700-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="ca700-128">Czego można się spodziewać?</span><span class="sxs-lookup"><span data-stu-id="ca700-128">What should I expect?</span></span>
<span data-ttu-id="ca700-129">Domyślnie aplikacja przykładowa umożliwia utworzenie słownika mapowania jednostek biznesowych na kody firm.</span><span class="sxs-lookup"><span data-stu-id="ca700-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="ca700-130">Każdy obiekt, który został zaksięgowany z polem **OwningBusinessUnit**, jest automatycznie ustawiany w celu używania określonej firmy.</span><span class="sxs-lookup"><span data-stu-id="ca700-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="ca700-131">Każdy podmiot bez pola **OwningBusinessUnit**, na przykład produkt, ustawi firmę na podstawie mapowania z pustą wartością jednostki biznesowej.</span><span class="sxs-lookup"><span data-stu-id="ca700-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="ca700-132">Aplikacja konsoli oczekuje jednego z parametrów: **-Symuluj** lub **-Zastosuj**.</span><span class="sxs-lookup"><span data-stu-id="ca700-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="ca700-133">Jeśli jest używany parametr wiersza polecenia **–symuluj**, nie są aktualizowane żadne dane.</span><span class="sxs-lookup"><span data-stu-id="ca700-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="ca700-134">Tylko **simulation_<entityname>.csv** są generowane w tym samym katalogu co narzędzie, po jednym dla każdej jednostki, która mogłaby zostać zaktualizowana.</span><span class="sxs-lookup"><span data-stu-id="ca700-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="ca700-135">Można iteracyjnie przeglądać te pliki podczas pracy, aby upewnić się, że kod firmy aktualizuje wartości w oczekiwany sposób.</span><span class="sxs-lookup"><span data-stu-id="ca700-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="ca700-136">Po zakończeniu pracy z symulowanymi aktualizacjami należy użyć parametru **–zastosuj**.</span><span class="sxs-lookup"><span data-stu-id="ca700-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="ca700-137">Powoduje zaktualizowanie wszystkich wierszy, w których obecnie występuje nieprawidłowa wartość firmy, w partiach wierszy w liczbie 1000 (domyślnie).</span><span class="sxs-lookup"><span data-stu-id="ca700-137">This updates all rows that currently have an incorrect company value, in batches of 1000 rows at a time (by default).</span></span> <span data-ttu-id="ca700-138">Kod jest nieżależny w podany sposób, co oznacza, że można go uruchomić ponownie i zaktualizować tylko niepoprawnie przypisane firmy.</span><span class="sxs-lookup"><span data-stu-id="ca700-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="ca700-139">W przypadku korzystania z opcji **-zastosuj**, kod wyprowadza pliki CSV wprowadzonych zmian o nazwie **applied_<entityname>.csv.**</span><span class="sxs-lookup"><span data-stu-id="ca700-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
