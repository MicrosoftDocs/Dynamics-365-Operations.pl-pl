...
Być może istnieje już istniejące Dataverse lub inne wystąpienie aplikacji Dynamics 365 z danymi biznesowymi, a użytkownik chce włączyć dla niego połączenie podwójnego odpisu. W tym przypadku, aby zainicjować firmę Dataverse lub inne dane aplikacji Dynamics 365 z informacjami o firmie przed włączeniem połączenia z podwójnym zapisywaniem.  
 
## <a name="when-should-i-use-bootstrapping"></a>Kiedy należy używać inicjowania firmy? 
Przed włączeniem map tabeli z podwójnym zapisywaniem (w kroku #5) należy wykonać ładowanie wstępne.  
1. Aby skonfigurować połączenie podwójnego zapisywania między wystąpieniami aplikacji Finance and Operations oraz z Dataverse lub z inną aplikacją Dynamics 365, zaloguj się do aplikacji Finance and Operations jako administrator. 
...
4. Włącz mapowanie tabeli **Cdm_companies**. Spowoduje to zsynchronizowanie firm z aplikacji Finance and Operations do Dataverse.  
    > [!div class="mx-imgBorder"]
    > ![Włącz mapowanie tabeli](media/dual-write-boot-2.png)
5. Należy uruchomić przykładowy kod inicjujący w wystąpieniu Dataverse lub innym wystąpieniu aplikacji Dynamics 365.  
6. Po zakończeniu inicjowania firmy i gdy system jest gotowy do synchronizacji na żywo, włącz mapowania tabeli.  

    Włączenie mapowań tabeli powoduje wyzwolenie początkowej synchronizacji danych dla włączonych mapowań tabeli. Dane odpowiadające firmom wybranym w połączeniu podwójnego zapisywania są synchronizowane między aplikacją Finance and Operations i Dataverse. 
 
## <a name="how-to-i-use-the-code-sample"></a>Jak mogę wykorzystać przykładowy kod?
Przykładowym kodem jest aplikacja C#, które można ładować w Visual Studio. Powoduje on zależności pakietów NuGet w zestawie SDK Dataverse, który można odświeżyć za pomocą narzędzia standardowego Visual Studio. 
...
Przykładowy kod pokazuje tylko jeden z wielu sposobów kategoryzowania wierszy jednostek według firm. Zmieniając logikę w sekcjach **DO ZROBIENIA**, można utworzyć kategoryzację niestandardową. 
...
Po zakończeniu pracy z symulowanymi aktualizacjami należy użyć parametru **–zastosuj**. Powoduje zaktualizowanie wszystkich wierszy, w których obecnie występuje nieprawidłowa wartość firmy, w partiach wierszy w liczbie 1000 (domyślnie). Kod jest nieżależny w podany sposób, co oznacza, że można go uruchomić ponownie i zaktualizować tylko niepoprawnie przypisane firmy. W przypadku korzystania z opcji **-zastosuj**, kod wyprowadza pliki CSV wprowadzonych zmian o nazwie **applied_<entityname>.csv.** 
