...
Może pojawić się następujący komunikat o błędzie podczas tworzenia wiersza w aplikacjach finansowych i operacyjnych:
...
Może pojawić się następujący komunikat o błędzie podczas próby zapisania danych tabeli w aplikacjach finansowych i operacyjnych:
...
Aby rozwiązać ten problem, należy upewnić się, że istnieją dane referencyjne wymagań wstępnych w aplikacjach finansowych i operacyjnych i Dataverse. Jeśli na przykład rekord klienta należy do konkretnej grupy klientów, należy upewnić się, że rekord grupy klientów istnieje w usłudze Dataverse.

Jeśli istnieją dane w obu miejscach i potwierdzono, że ten błąd nie jest związany z danymi, wykonaj następujące kroki.

1. Otwórz encję **DualWriteProjectConfigurationEntity** przy użyciu dodatku programu Excel. Aby użyć tego dodatku, włącz tryb projektowania w dodatku programu Excel aplikacji finansowych i operacyjnych i dodaj do arkusza encję **DualWriteProjectConfigurationEntity**. Więcej informacji można znaleźć w temacie [Wyświetlanie i aktualizowanie danych jednostki przy użyciu programu Excel](../../office-integration/use-excel-add-in.md).
...
## <a name="handle-read-or-write-privilege-errors-when-you-create-data-in-a-finance-and-operations-app"></a>Obsługa błędów uprawnień do odczytu lub odczytu podczas tworzenia danych w aplikacjach finansowych i operacyjnych

Może pojawić się komunikat o błędzie „Nieprawidłowe żądanie” podczas tworzenia danych w aplikacjach finansowych i operacyjnych.
...
1. W aplikacjach finansowych i operacyjnych znajdź jednostkę biznesową, która jest zamapowana w zestawie połączenia integracji danych.
...
Może pojawić się następujący komunikat o błędzie podczas tworzenia danych w aplikacjach finansowych i operacyjnych:
...
Ten błąd występuje, gdy środowisko Dataverse jest niepoprawnie resetowane podczas próby utworzenia danych w aplikacjach finansowych i operacyjnych.
...
Aby rozwiązać ten problem, musisz wykonać kroki w usłudze Dataverse i w aplikacjach finansowych i operacyjnych.

1. W aplikacjach finansowych i operacyjnych wykonaj następujące kroki:

    1. Otwórz encję **DualWriteProjectConfigurationEntity** przy użyciu dodatku programu Excel. Aby użyć tego dodatku, włącz tryb projektowania w dodatku programu Excel aplikacji finansowych i operacyjnych i dodaj do arkusza encję **DualWriteProjectConfigurationEntity**. Więcej informacji można znaleźć w temacie [Wyświetlanie i aktualizowanie danych jednostki przy użyciu programu Excel](../../office-integration/use-excel-add-in.md).
    2. Wybierz i usuń rekordy, które mają problemy w mapie i projekcie podwójnego zapisu. Dla każdego mapowania podwójnego zapisu będą dostępne dwa rekordy.
    3. Opublikuj zmiany przy użyciu dodatku programu Excel. Ten krok jest ważny, ponieważ powoduje usunięcie rekordów z encji i tabel źródłowych.
    4. Aby zapobiec błędom po ponownym połączeniu środowisk aplikacji finansowych i operacyjnych lub Dataverse, należy upewnić się, że nie pozostają żadne konfiguracje podwójnego zapisu.
...
3. W aplikacjach finansowych i operacyjnych wykonaj następujące kroki:
...
## <a name="data-from-finance-and-operations-apps-isnt-synced-to-dataverse"></a>Dane z aplikacji finansowych i operacyjnych nie są synchronizowane do Dataverse

Podczas synchronizacji na żywo może wystąpić problem, gdy tylko część danych jest synchronizowana z aplikacji finansowych i operacyjnych do usługi Dataverse lub dane w ogóle nie są synchronizowane.
...
+ Zdarzenia biznesowe muszą być zarejestrowane dla źródła danych, które jest mapowane. Niektóre źródła danych mogą używać sprzężenia zewnętrznego i mogą być oznaczone jako tylko do odczytu w aplikacjach finansowych i operacyjnych. Te źródła danych nie są śledzone.
...
1. Przejrzyj mapowania pól na stronie administrowania podwójnym zapisem. Jeśli pole nie jest mapowane w aplikacjach finansowych i operacyjnych do usługi Dataverse, nie będzie śledzone. Na przykład na poniższej ilustracji pole **Opis** jest śledzone z usługi Dataverse, ale nie w aplikacjach finansowych i operacyjnych. Zmiany tego pola w aplikacjach finansowych i operacyjnych nie będą śledzone.
...
## <a name="error-when-you-create-a-record-where-multiple-records-are-sent-from-a-finance-and-operations-app-to-dataverse-in-the-same-batch"></a>Błąd podczas tworzenia rekordu, w którym wiele rekordów jest wysyłanych z w aplikacjach finansowych i operacyjnych do usługi Dataverse w tej samej partii

W przypadku każdej transakcji w aplikacjach finansowych i operacyjnych tworzy dane w partii i wysyła je jako partię do usługi Dataverse. Jeśli w ramach tej samej transakcji utworzone zostaną dwa rekordy i odwołują się one do siebie, może zostać wyświetlony komunikat o błędzie przypominający następujący przykład w aplikacjach finansowych i operacyjnych:
...
Aby rozwiązać ten problem, utwórz w aplikacjach finansowych i operacyjnych relacje encji, aby wskazać, że te dwie encje są ze sobą powiązane oraz że powiązane rekordy są obsługiwane w tej samej transakcji.

## <a name="enable-verbose-logging-of-error-messages"></a>Włącz pełne rejestrowanie komunikatów o błędach

W aplikacjach finansowych i operacyjnych mogą wystąpić błędy związane ze środowiskiem usługi Dataverse. Komunikat o błędzie może nie zawierać pełnego tekstu wiadomości lub innych odpowiednich danych. Aby uzyskać więcej informacji, można włączyć pełne rejestrowanie, ustawiając flagę **IsDebugMode** dostępną w encji **DualWriteProjectConfigurationEntity** we wszystkich konfiguracjach projektów w aplikacjach finansowych i operacyjnych.
...
Podczas próby dodania adresu klienta lub kontaktu w aplikacjach finansowych i operacyjnych lub usłudze Dataverse może pojawić się następujący komunikat o błędzie:
...
Po utworzeniu klienta w usłudze Dataverse jest generowany nowy numer strony. Komunikat o błędzie jest wyświetlany, gdy rekord klienta oraz strona są synchronizowane z aplikacjami finansowymi i operacyjnymi, ale istnieje już rekord klienta o innym numerze strony.
...
W usłudze Dataverse istnieje sekwencja numerów tabeli **msdyn_party**. Podczas tworzenia konta w usłudze Dataverse jest tworzona nowa strona (na przykład **Strona-001** typu **Organizacja**). Te dane są wysyłane do aplikacji finansowych i operacyjnych. Jeśli środowisko Dataverse zostanie zresetowane lub środowisko w aplikacjach finansowych i operacyjnych zostanie połączone z innym środowiskiem Dataverse, a następnie zostanie utworzony nowy rekord osoby kontaktowej w usłudze Dataverse, zostanie utworzona nowa wartość strony, która zaczyna się od **Strona-001**. Tym razem utworzony rekord strony to **Strona-001** typu **Osoba**. Po zsynchronizowaniu tych danych w aplikacjach finansowych i operacyjnych będą wyświetlać poprzedni komunikat o błędzie, ponieważ rekord strony **Strona-001** typu **Organizacja** już istnieje.
