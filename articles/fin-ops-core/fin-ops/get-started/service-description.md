---
title: Opis usługi dla aplikacji finansowych i operacyjnych
description: Ten artykuł zawiera opis usługi dla aplikacji finansowych i operacyjnych.
author: tomhig
ms.date: 04/27/2022
ms.topic: article
...
Aplikacje finansowe i operacyjne są produktami planowania zasobów przedsiębiorstwa (ERP) udostępnianymi w modelu „oprogramowanie jako usługa” (SaaS), które są oparte na platformie [Microsoft Azure](https://azure.microsoft.com/overview/what-is-azure/) i do niej przeznaczone. Aplikacje finansowe i operacyjne zapewnia organizacjom funkcje systemu ERP, które spełniają ich indywidualne wymagania i pomagają im adaptować się do stale zmieniającego się środowiska biznesowego, bez konieczności zarządzania infrastrukturą. Aplikacje finansowe i operacyjne mogą zawierać dowolną liczbę następujących obszarów rozwiązania:
...
- Standardowa gotowa konfiguracja aplikacji finansowych i operacyjnych (bez rozszerzeń)
- Konfiguracja aplikacji finansowych i operacyjnych, która zawiera dowolną liczbę rozwiązań oprogramowania ISV
- Konfiguracja aplikacji finansowych i operacyjnych, która zawiera jedno lub więcej rozszerzeń specyficznych dla klienta
- Konfiguracja aplikacji finansowych i operacyjnych, która zawiera zarówno specjalne rozszerzenia klienta, jaki dowolną liczbę rozwiązań oprogramowania ISV
...
Model działania aplikacji finansowych i operacyjnych wyznacza klientowi, partnerowi wdrożeniowemu i firmie Microsoft określone role i zakresy odpowiedzialności w trakcie cyklu życia usługi. Aby uzyskać więcej informacji, zobacz [Operacje i obsługa w chmurze](../../dev-itpro/lifecycle-services/cloud-operations-servicing.md).
...
Aplikacje finansowe i operacyjne są skalowane zależnie od wolumenu transakcji i obciążenia użytkownika. Każda implementacja klienta tworzy unikatowe rozwiązanie, które składa się z następujących elementów:
...
- Analizowanie i rozwiązywanie problemów z wydajnością systemu związanych z aplikacjami finansowymi i operacyjnymi

Aby konfiguracja zapewniała wysoką wydajność, klienci muszą wykonać następujące zadania:

- Podanie dokładnych informacji dotyczących użycia implementacji aplikacji finansowych i operacyjnych w narzędziu do szacowania zapotrzebowania na [subskrypcje usługi LCS](../../dev-itpro/lifecycle-services/subscription-estimator.md).
...
Aplikacje finansowe i operacyjne są obsługiwane z różnych regionów świadczenia usługi Azure na świecie. Aplikacje finansowe i operacyjne zawierają funkcje służące do obsługi różnych krajów/regionów oraz języków ojczystych. Aby uzyskać więcej informacji, zobacz [Funkcje związane z lokalizacją i wymagane przepisami prawa](../../dev-itpro/lcs-solutions/country-region.md#localization-and-regulatory-features).

### <a name="countryregion-specific-considerations"></a>Uwagi dotyczące niektórych krajów/regionów

- Klienci działające w branży regulowanej lub organizacje handlowe prowadzące interesy z podmiotami z Francji, które wymagają lokalnego przechowywania danych powinni zapoznać się z tematem [Aplikacje finansowe i operacyjne we Francji](../../dev-itpro/deployment/france-local-deployment.md).
- Klienci prowadzący działalność w Chinach powinni zapoznać się z tematami [Podręcznik Azure China](/azure/china/) oraz [Aplikacje finansowe i operacyjne — obsługiwane przez firmę 21Vianet w Chinach](../../dev-itpro/deployment/china-local-deployment.md).
...
W aplikacjach finansowych i operacyjnych Microsoft pełni rolę procesora. Jako podmiot przetwarzający dane dostarcza w aplikacjach finansowych i operacyjnych procesy i funkcje, które pomagają klientom wywiązywać się z obowiązków wynikających z roli administratora danych w rozumieniu RODO. Aby uzyskać więcej informacji, zobacz temat [Omówienie RODO](../../dev-itpro/gdpr/gdpr-guide.md).
...
**Zdarzenie:** [żądanie wystąpienia produkcyjnego](../imp-lifecycle/go-live-faq.md#when-can-i-configure-and-request-my-production-environment)

- Wypełnienie [listy przygotowania rozpoczynania eksploatacji](../imp-lifecycle/prepare-go-live.md) i przesłanie jej zespołowi [Microsoft FastTrack](/dynamics365/fasttrack/).
...
W rozumieniu [zasad cyklu życia oprogramowania](../../dev-itpro/migration-upgrade/versions-update-policy.md) aplikacje aplikacji finansowych i operacyjnych podlegają zasadom [Modern Lifecycle Policy](../../dev-itpro/migration-upgrade/versions-update-policy.md#modern-lifecycle-policy) firmy Microsoft jako produkty serwisowane i obsługiwane w trybie ciągłym. 

Firma Microsoft co roku wydaje osiem aktualizacji usługi aplikacji finansowych i operacyjnych w następujących miesiącach:
...
Dostęp administracyjny do środowiska produkcyjnego aplikacji finansowych i operacyjnych jest ściśle kontrolowany i rejestrowany. Dane klientów są obsługiwane zgodnie z [warunkami świadczenia usług online firmy Microsoft](https://www.microsoft.com/licensing/terms/productoffering). 
...
Firma Microsoft zapewnia ciągłość biznesową i odzyskiwanie po awarii wystąpień produkcyjnych aplikacji finansowych i operacyjnych w przypadku awarii w całym regionie świadczenia usługi Azure. Aby uzyskać więcej informacji, w tym na temat celu czasu przywracania usługi (Recovery Time Objective – RTO) i celu punktu przywracania usługi (Recovery Point Objective – RPO), zobacz [Ciągłość działania i odzyskiwanie danych po awarii](../../dev-itpro/sysadmin/business-continuity-disaster-recovery.md).
...
Jeśli odzyskiwania po awarii zostanie użyte odzyskanie wystąpienia produkcyjnego klienta, firma Microsoft i klient będą wykonywać swoje obowiązki w zakresie [zarządzania zdarzeniami](service-description.md#incident-management).

Plany i procedury odzyskiwania po awarii firmy Microsoft są regularnie badane w audytach System and Organization Controls (SOC). Te audyty zgodności atestują proces techniczny i proceduralny odzyskiwania po awarii firmy Microsoft, w aplikacjach finansowych i operacyjnych Dynamics 365. Raporty audytów [zgodności SOC](/compliance/regulatory/offering-soc-2) i wszystkie inne raporty zgodności są dostępne w [ofercie rozwiązań do zapewniania zgodności Centrum zaufania Microsoft](/compliance/regulatory/offering-home).

## <a name="finance-and-operations-support-offerings"></a>Pomoc: aplikacje finansowe i operacyjne

Pomoc techniczna jest dostępna na rynkach, na których są oferowane usługi aplikacji finansowych i operacyjnych. [Doświadczenia wsparcia](../../dev-itpro/lifecycle-services/lcs-support.md) są dostępne w aplikacjach LCS lub aplikacji finansowych i operacyjnych. Oto kilka przykładów:

- [Wyszukiwanie problemów](../../dev-itpro/lifecycle-services/issue-search-lcs.md) w LCS
- [Zintegrowane wsparcie techniczne](../../dev-itpro/lifecycle-services/support-experience.md) w aplikacjach finansowych i operacyjnych
- [Pomoc techniczna w chmurze](../../dev-itpro/lifecycle-services/cloud-powered-support-lcs.md) w usłudze LCS

Firma Microsoft oferuje klientom aplikacji finansowych i operacyjnych trzy plany pomocy technicznej: Premier, Professional Direct oraz pomoc techniczną zawartą w subskrypcji. Poziom pomocy technicznej różni się w zależności od planu. W poniższej tabeli przedstawiono porównanie tych trzech planów.
...
W przypadku zdarzeń, które są związane z aplikacjami finansowymi i operacyjnymi, klienci przesyłają bilety pomocy technicznej do firmy Microsoft za pośrednictwem usługi LCS. CSS obsługuje zdarzenia na podstawie planu pomocy technicznej klienta oraz ważności zdarzenia przypisanej przez CSS.
...
- **[Centrum zaufania](https://www.microsoft.com/trust-center)** — zawiera informacje o miejscu przechowywania danych aplikacji finansowych i operacyjnych oraz dodatkowe informacje o procedurach dotyczących prywatności, zgodności i zabezpieczeń.
...
Narzędzie w usługach LCS, które pomaga wykonać analizę luk zdatności danej implementacji za pomocą definicji procesów biznesowych z American Productivity & Quality Center (APQC), które są obsługiwane w aplikacjach finansowych i operacyjnych.
...
### <a name="customer"></a>Odbiorca

Osoba prawna, który zużywa aplikacje finansowe i operacyjne i jest reprezentowana przez dzierżawę w programie Office 365.

### <a name="development-environment"></a>Środowisko projektowe
...
Partner wybrany przez klienta w celu dostosowywania, konfigurowania, implementowania i zarządzania jego rozwiązaniami aplikacji finansowych i operacyjnych.

### <a name="incident"></a>Zdarzenie

Problem, który klienci napotkają podczas korzystania z aplikacji finansowych i operacyjnych, i na temat którego przesyłają bilet za pośrednictwem usługi LCS.

### <a name="microsoft-customer-support-services-css"></a>Usługi pomocy technicznej firmy Microsoft (CSS)

Globalny zespół pomocy technicznej firmy Microsoft, który jest dedykowany do pilnowania jakości usługi aplikacji finansowych i operacyjnych.

### <a name="microsoft-dynamics-lifecycle-services-lcs"></a>Microsoft Dynamics Lifecycle Services (LCS)

Portal administracyjny do zarządzania cyklem życia aplikacje finansowych i operacyjnych od wersji próbnej, przez implementację, do zarządzania poprodukcyjnego i pomocy technicznej. Aby uzyskać więcej informacji, zobacz [Zasoby w Lifecycle Services (LCS)](../../dev-itpro/lifecycle-services/lcs.md).
...
Środowisko aplikacji finansowych i operacyjnych używane przez klienta do zarządzania codziennymi transakcjami i procesami biznesowymi „na żywo”.
...
### <a name="service"></a>Obsługa klienta

Wszystkie podstawowe usługi, które są zawarte w aplikacjach finansowych i operacyjnych.
...
Firma Microsoft regularnie serwisuje środowiska aplikacji finansowych i operacyjnych, dokonując aktualizacji usług. Klienci, w zależności od własnych potrzeb biznesowych, wyznaczają własny kalendarz aktualizacji usług. Aby uzyskać więcej informacji, zobacz [Aktualizacje usługi One Version](../../dev-itpro/lifecycle-services/oneversion-overview.md).
