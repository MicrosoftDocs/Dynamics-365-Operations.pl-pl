...
Aby dodać jednostkę magazynową do klucza alokacji produktów, przejdź do okna **Planowanie główne \> Ustawienia \> Prognozowanie popytu \> Klucze alokacji produktu**. Na stronie **przypisywania towarów** przypisz produkty do klucza alokacji.
...
Wybierając **Wymiary prognozy** w lewym okienku na stronie **Parametry prognozowania popytu**, możesz też wybrać zestaw wymiarów prognozy, które mają być używane przy generowaniu prognozy popytu. Wymiar prognozy wskazuje poziom szczegółów, które dla którego prognoza jest definiowana. Firma, oddział i klucz alokacji produktów są wymagane w wymiarach prognozy, ale można również tworzyć prognozy na poziomie magazynu, stanu zapasów, grupy odbiorców, konta odbiorców, kraju/regionu, stanu i towaru oraz wymiaru wszystkich towarów.
...
Nie wszystkie towary działają w taki sam sposób z perspektywy prognozowania popytu. Podobne towary mogą być pogrupowane w jeden klucz alokacji produktów i parametry, takie jak typy transakcji i ustawienia metod prognozy można ustawić według klucza alokacji produktów. Wybierz **Klucze alokacji produktów** w lewym okienku na stronie **Parametry prognozowania popytu**.
...
Aby wyświetlić parametry, które można skonfigurować na potrzeby usługi prognozowania popytu, wybierz kolejno opcje **Planowanie główne \> Ustawienia \> Prognozowanie popytu \> Parametry algorytmu prognozowania**. Strona **Domyślne parametry algorytmu prognozowania** zawiera wartości domyślne parametrów. Parametry te można zastąpić, przechodząc do obszaru **Planowanie główne \> Ustawienia \> Prognozowanie popytu \> Parametry prognozowania popytu**, w którym można wykonać następujące czynności:

- Zastąpić parametry globalnie na karcie **Ogólne**.
- Zastąpić parametry dla poszczególnych kluczy alokacji na karcie **Klucze alokacji produktów**. Parametry, które są zastępowane dla klucza alokacji produktów mają wpływ tylko prognozę towarów, które są skojarzone z tym kluczem alokacji towaru.

### <a name="forecast-algorithm-parameters"></a>Parametry algorytmu prognozy

Na karcie **Klucze alokacji produktów** na stronie **Parametry prognozowania popytu** można użyć skróconej karty **parametrów algorytmu prognozy**, aby przypisać parametry algorytmu prognozy dla klucza alokacji produktów obecnie zaznaczonego w siatce po lewej stronie. Użyj przycisków **Dodaj** i **Usuń** na pasku narzędzi, aby określić wymaganą kolekcję parametrów. Dla każdego parametru z listy wybierz jedną z następujących wartości w polu **Nazwa**, a następnie wprowadź odpowiednią wartość w polu **Wartość**:
