...
- Mapowanie pola **CustomerGroupId** z **CDS &gt; Miejsce docelowe** musi zostać zaktualizowane o prawidłową wartość w programie Finance and Operations. Można określić wartość domyślną albo ustawiać wartość przy użyciu mapy wartości. Wartość domyślna w szablonie to **10**.
- Pole **Kod kraju/regionu w adresie** jest wymagane w programie Finance and Operations. Aby uniknąć błędów synchronizacji, można określić wartość domyślną w mapowaniu z **CDS &gt; Miejsce docelowe**. Ta wartość domyślna będzie następnie używana, jeśli to pole jest pozostawione puste w aplikacji Sales.
