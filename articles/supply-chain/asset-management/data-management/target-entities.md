---
title: Jednostki docelowe
description: W tym artykule opisano sposób uzyskiwania przeglądu jednostek docelowych w Zarządzaniu składnikami majątku.
author: johanhoffmann
ms.date: 09/26/2019
ms.topic: article
ms.prod: ''
ms.technology: ''
ms.search.form: ''
audience: Application User
ms.reviewer: kamaybac
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: johanho
ms.search.validFrom: 2019-08-31
ms.dyn365.ops.version: 10.0.5
ms.openlocfilehash: 838e3affd4e782bea463638e28da3943329671d4
ms.sourcegitcommit: 6781fc47606b266873385b901c302819ab211b82
ms.translationtype: HT
ms.contentlocale: pl-PL
ms.lasthandoff: 07/02/2022
ms.locfileid: "9111865"
---
# <a name="target-entities"></a>Jednostki docelowe

[!include [banner](../../includes/banner.md)]

 

W obszarze roboczym **Zarządzanie danymi** można uzyskać przegląd jednostek docelowych, powiązanych typów jednostek i tabel przemieszczania związanych z modułem **Zarządzanie składnikami majątku**. 

1. Kliknij przycisk **aplikacji finansowych i operacyjnych** w lewym górnym rogu ekranu, a następnie kliknij pozycję **Zarządzanie danymi**.

2. W sekcji **Import/eksport** kliknij pozycję **Jednostki danych**. 

    ![Rysunek 1.](media/01-data-management.png)

3. Na stronie **Jednostki docelowe** wyszukaj hasło „zarządzanie skłądnikami majątku” w filtrze i naciśnij klawisz ENTER, aby wyświetlić listę jednostek związanych z zarządzaniem składnikami majątku.

    Na poniższym rysunku przedstawiono niektóre jednostki zarządzania składnikami majątku.

   ![Rysunek 2.](media/02-data-management.png)

4. Wybierz jednostkę i kliknij przycisk **Modyfikuj mapowanie celu**.

5. Na stronie **Mapuj dane tymczasowe do celu** wyświetlana jest lista pól przemieszczania powiązanych z wybraną jednostką. Kliknij **Wizualizacja mapowania**, aby wyświetlić graficzne omówienie pojęć związanych z danymi pośrednimi i docelowymi. 

    Na poniższym rysunku przedstawiono wizualizację pól związanych z jednostką typy typu składnika majątku.

    ![Rysunek 3.](media/03-data-management.png)



[!INCLUDE[footer-include](../../../includes/footer-banner.md)]
