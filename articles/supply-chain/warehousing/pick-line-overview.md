...
1. Na karcie skróconej **Ogólne** skonfiguruj funkcję, ustawiając wartość pola **Pokaż listę wierszy pracy** na jedną z następujących wartości:
    ...
    - **Pokaż tylko w przypadku żądania** — pracownicy mogą wyświetlić listę wierszy pobrania, wybierając przycisk **Przejdź do** w aplikacji Warehouse Management.
    - **Pokazuje listę na początku każdego pobrania** — umożliwia wyświetlenie listy za każdym razem po rozpoczęciu lub zakończeniu wiersza pobrania. Można również ponownie wyświetlić listę, wybierając przycisk **Przejdź do** w aplikacji Warehouse Management.
    - **Pokaż tylko na początku pierwszego pobrania** – Pracownicy widzą listę za każdym razem, gdy rozpoczynają nową pracę pobrania, ale nie po każdym wierszu. Można również ponownie wyświetlić listę, wybierając przycisk **Przejdź do** w aplikacji Warehouse Management.
    - **Nigdy nie pokazuj** — w aplikacji Warehouse Management pojawia się standardowy przycisk **Pomiń**, a wyświetlanie listy wierszy pracy jest wyłączone. Przycisk **Pomiń** umożliwia pracownikom przechodzenie między wierszami pojedynczo w ustalonej kolejności. Mogą również przewijać listę tyle razy, ile potrzebują, aż wszystkie wiersze zostaną przetworzone.
...
1. Na stronie **Lista pól** skonfiguruj informacje, które aplikacja Warehouse Management będzie wyświetlać dla każdego wiersza na liście.
