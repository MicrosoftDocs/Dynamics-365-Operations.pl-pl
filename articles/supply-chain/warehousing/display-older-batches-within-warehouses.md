...
- <span data-ttu-id="1feeb-110">Kontynuowanie pobierania z bieżącej lokalizacji — W przypadku dalszego używania bieżącej lokalizacji wiersza pracy ilości dla wiersza pracy będą w dalszym ciągu pobierane z oryginalnej lokalizacji.</span><span class="sxs-lookup"><span data-stu-id="1feeb-110">Continue picking from the current location - If you continue with the current work line location, the quantities for the work line will continue to be picked from the original location.</span></span>

## <a name="where-it-applies"></a><span data-ttu-id="1feeb-111">Zastosowanie</span><span class="sxs-lookup"><span data-stu-id="1feeb-111">Where it applies</span></span>
<span data-ttu-id="1feeb-112">Funkcjonalność wyświetlania starszych partii w magazynie jest konfigurowana w menu urządzenia przenośnego i wpływa na pobieranie towarów w partiach poniżej.</span><span class="sxs-lookup"><span data-stu-id="1feeb-112">Display older batches within warehouse is configured on mobile device menu items and affects the pick for batch below items.</span></span>

## <a name="set-up-display-older-batches-within-warehouse"></a><span data-ttu-id="1feeb-113">Konfigurowanie wyświetlania starszych partii w magazynie</span><span class="sxs-lookup"><span data-stu-id="1feeb-113">Set up Display older batches within warehouse</span></span>
<span data-ttu-id="1feeb-114">Ustawienie **Wyświetl starsze partie w magazynie** jest dostępne w elementach menu urządzenia przenośnego, gdy w opcji **Pobierz z najstarszej partii** ustawiono wartość **Ostrzegaj**.</span><span class="sxs-lookup"><span data-stu-id="1feeb-114">The **Display older batches within warehouse** configuration is available on mobile device menu items when the **Pick oldest batch** option is set to **Warn**.</span></span>

- <span data-ttu-id="1feeb-115">W oknie **Zarządzanie magazynem** > **Ustawienia** > **Urządzenie przenośne** > **Elementy menu urządzenia przenośnego** w opcji **Użyj istniejącej pracy** ustaw wartość **Tak**, a następnie w polu **Pobierz z najstarszej partii** ustaw wartość **Ostrzegaj**.</span><span class="sxs-lookup"><span data-stu-id="1feeb-115">Under **Warehouse management** > **Setup** > **Mobile device** > **Mobile device menu items**, set **Use existing work** to **Yes** for the menu item, and select **Warn** in the **Pick oldest batch** field.</span></span> 
